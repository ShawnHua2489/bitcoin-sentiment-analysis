...
def main():
    ...
    # Collect Trump posts from Truth Social
    print("\nCollecting Trump posts from Truth Social...")
    truth_collector = TruthCollector()
    truth_df = truth_collector.get_trump_posts(start_date=start_date)
    ...
    # Analyze impact of Truth Social posts
    if not truth_df.empty and not price_df.empty:
        print("\nAnalyzing impact of Trump Truth Social posts on Bitcoin price...")
        results = analyze_truth_impact(truth_df, price_df)
    
    # Print summary statistics
    print("\nSummary Statistics:")
    if not truth_df.empty:
        print(f"Total Truth Social posts analyzed: {len(truth_df)}")
    print(f"Time window analyzed: 6 hours before and after each post")
